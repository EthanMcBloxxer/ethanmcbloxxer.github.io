...
3. Run the following commands in Terminal:
    1. `hdiutil create -o /tmp/MacBigSur -size 12500m -volname MacBigSur -layout SPUD -fs HFS+J`{:.language-lua}
    2. `hdiutil attach /tmp/MacBigSur.dmg -noverify -mountpoint /Volumes/MacBigSur`{:.language-lua}
    3. `sudo /Applications/Install\ macOS\ Big\ Sur/Contents/Resources/createinstallmedia --volume /Volumes/MacBigSur --nointeraction`{:.language-lua}
    4. `hdiutil detach /Volumes/MacBigSur/`{:.language-lua}
    5. `hdiutil convert /tmp/MacBigSur.dmg -format UDTO -o ~/Desktop/MacBigSur.cdr`{:.language-lua}
    6. `mv ~/Desktop/MacBigSur.cdr ~/Desktop/BigSur.iso`{:.language-lua}
